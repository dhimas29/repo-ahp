...
if ($modul == 'pos' && $ac == 'kirim') {
    ...
    try {
        ...
        $mail->Body    = 'Halo ' . $row['nama_lengkap'] . '<br><br>
            Berdasarkan test yang sudah dikerjakan anda di nyatakan <h2><span style="color:red">LOLOS</span></h2>
            <p>Silahkan klik link berikut untuk konfirmasi anda </p>
            <a target="_blank" href="localhost/skripsi/proses/prosesubah.php?module=konfirmasi_pelamar&act=kirim&email=' . $email . '">Klik disini</a>';

        $mail->send();
        echo "<script>alert('Berhasil Mengirim Email'); window.location.href='../adminweb/index.php?page=laporan_ranking'; </script>";
    ...
    try {
        ...
        $mail->Body    = 'Halo ' . $fullname . '<br><br>
            ...
            <a target="_blank" href="localhost/skripsi/proses/prosesubah.php?module=konfirmasi&act=kirim&email=' . $email . '">Klik disini</a>';
